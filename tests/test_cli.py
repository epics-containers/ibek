...
def test_ibek_schema(tmp_path: Path, samples: Path):
    ...
    run_cli("support", "generate-schema", schema_path)
    expected = json.loads(
...
def test_object_schema(tmp_path: Path, samples: Path):
    ...
    run_cli("ioc", "generate-schema", yaml_path, schema_path)
...
def test_ioc_schema(tmp_path: Path, samples: Path):
    ...
    run_cli("ioc", "generate-schema", yaml_path1, yaml_path2, schema_combined)
